...
public class DeptCopyController {
	...
	//특정 부서 번호로 검색
	//http://localhost/guestbook/deptone?deptno=10   존재하는 부서 번호 검색
	
	//http://localhost/guestbook/deptone?deptno=100  미존재하는 부서 번호 검색시의 해결
...
//		System.out.println(dao.findById(deptno).get()); //데이터가 없을 경우 실행 예외 발생
		...
		/* 부서번호 존재시 : DeptCopy(deptno=10, dname=ACCOUNTING, loc=NEW YORK)
		 * 부서번호 미 존재시 : 데이터 무 
		 * 개발자 관점에서 확인용 코드로 간주 */
